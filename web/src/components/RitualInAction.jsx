...
// Composition mirrors the canonical ritual in ritualVideo.js.
// Daily: 04, 01, 03, 08, 07. Weekly: 05, 06, 04, 02, 06 finish.
const STEPS = [
...
.ria-ritual-overview{max-width:1120px;margin:0 auto 32px;padding:0 24px;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;}
.ria-ritual-card{border:1px solid rgba(240,236,226,.14);background:rgba(24,28,36,.58);padding:22px;position:relative;overflow:hidden;}
.ria-ritual-card::before{content:"";position:absolute;inset:0;background:linear-gradient(135deg,rgba(74,143,199,.14),rgba(8,9,11,0) 48%);opacity:.35;pointer-events:none;}
.ria-ritual-card.active{border-color:rgba(74,143,199,.56);background:linear-gradient(135deg,rgba(18,35,58,.9),rgba(8,9,11,.76));}
.ria-ritual-card.weekly.active{border-color:rgba(200,169,110,.5);}
...
.ria-ritual-title{font-family:'Bebas Neue',sans-serif;font-size:30px;letter-spacing:.06em;color:var(--bone);line-height:1;}
...
.ria-ritual-card.weekly .ria-ritual-cadence{color:#c8a96e;}
.ria-ritual-summary{position:relative;margin:0 0 16px;color:rgba(240,236,226,.72);font-size:15px;line-height:1.45;font-weight:300;}
.ria-ritual-promise{position:relative;margin:0 0 18px;color:var(--bone);font-size:16px;line-height:1.45;}
.ria-ritual-sequence{position:relative;list-style:none;margin:0;padding:0;display:grid;gap:9px;}
.ria-ritual-step{display:grid;grid-template-columns:auto 1fr;gap:8px 10px;align-items:start;padding-top:9px;border-top:1px solid rgba(240,236,226,.1);}
.ria-ritual-step:first-child{padding-top:0;border-top:none;}
.ria-step-num{grid-row:1 / span 2;font-family:'Bebas Neue',sans-serif;font-size:13px;letter-spacing:.09em;color:var(--bone);border:1px solid rgba(240,236,226,.26);padding:2px 7px;background:rgba(8,9,11,.38);}
...
.ria-ritual-action{position:relative;margin-top:18px;border:1px solid rgba(240,236,226,.22);background:transparent;color:var(--bone);font-family:'Bebas Neue',sans-serif;letter-spacing:.1em;font-size:14px;padding:11px 18px;cursor:pointer;}
.ria-ritual-action:hover,.ria-ritual-action:focus-visible{border-color:var(--bone);outline:none;}
.ria-ritual-card.active .ria-ritual-action{background:var(--bone);color:var(--black);border-color:var(--bone);}
...
@media(max-width:768px){
  ...
  .ria-ritual-overview{grid-template-columns:1fr;gap:12px;margin-bottom:26px;}
...
export default function RitualInAction() {
  const [activeRitual, setActiveRitual] = useState('daily');
  const [activeIdx, setActiveIdx] = useState(0);
  const [userSelected, setUserSelected] = useState(false);
  const [mediaActivated, setMediaActivated] = useState(false);
  const [loadedMedia, setLoadedMedia] = useState(() => new Set());
  const visibleSteps = useMemo(() => STEPS.filter((step) => step.ritual === activeRitual), [activeRitual]);
  ...
  const settle = useCallback((i) => {
    ...
    capture('ritual_selected', { product: step.slug, ritual: step.ritual, source: 'ritual_in_action' });
  }, [loadMedia, visibleSteps]);

  const chooseRitual = useCallback((ritualKey) => {
    if (ritualKey === activeRitual) return;
    vidRefs.current.forEach((v) => v?.pause());
    setActiveRitual(ritualKey);
    setActiveIdx(0);
    setUserSelected(false);
    setLoadedMedia(new Set());
    activeIdxRef.current = 0;
    firstSettle.current = true;
    progressFired.current = new Set();
    cardRefs.current = [];
    vidRefs.current = [];
    carouselRef.current?.scrollTo?.({ left: 0, behavior: 'smooth' });
    capture('ritual_mode_selected', { ritual: ritualKey, source: 'ritual_in_action' });
  }, [activeRitual]);
  ...
  }, [mediaActivated, loadMedia, activeRitual]);
    ...
    <>
      ...
      <section className="ria-section" id="ritual" ref={sectionRef}>
        ...
        <div className="ria-ritual-overview reveal">
          {RITUALS.map((ritual) => (
            <article
              className={`ria-ritual-card ${ritual.key}${activeRitual === ritual.key ? ' active' : ''}`}
              key={ritual.key}
            >
              ...
              <button className="ria-ritual-action" type="button" onClick={() => chooseRitual(ritual.key)}>
                {ritual.cta}
              </button>
            </article>
          ))}
