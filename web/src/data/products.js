export const PRODUCTS = [
  {
    num: '01',
    name: 'Body Wash',
    fullName: 'Skin Protect Body Wash 250ml',
    origin: '🇬🇧 United Kingdom',
    tag: 'Body · Daily',
    tagline: 'Gentle on you.',
    benefits: [
      'Gentle for every part of your body',
      'Protects your skin barrier',
      'Sulphate free',
      'Leaves skin balanced',
      'Calms as it cleans',
    ],
    desc: 'Amino acid surfactants. No sulphates. Cleans without stripping. Skin stays balanced, never tight.',
    image: '/products/01-body-wash.png',
    replacementCycleDays: 60,
    comingSoon: false,
  },
  {
    num: '02',
    name: 'Italy Towel Mitt',
    fullName: 'Italy Towel Mitt',
    origin: '🇰🇷 Korean Tradition',
    tag: 'Body · Weekly',
    tagline: 'Deep clean. Every week.',
    benefits: [
      'Used weekly, not daily. Your skin needs recovery time.',
      '100% viscose for the deepest clean possible',
      'Removes dead skin that body wash can\'t reach',
      'First time you use it, you\'ll see the difference',
    ],
    desc: 'Inspired by Korean bathhouses. 100% viscose material engineered for the deepest clean possible. Removes what body wash alone can never reach.',
    image: '/products/02-italy-towel-mitt.png',
    replacementCycleDays: 60,
    comingSoon: false,
  },
  {
    num: '03',
    name: 'Back Scrub Cloth',
    fullName: 'Back Scrub Cloth 70cm',
    origin: '🇰🇷 Korean Tradition',
    tag: 'Back · Daily',
    tagline: 'Your back. Finally clean.',
    benefits: [
      'The only tool that properly reaches your entire back',
      '100% viscose with dual texture for effective exfoliation',
      'Removes dead skin and buildup body wash never touches',
      'One handle each hand, 60 seconds. Back done.',
    ],
    desc: '70cm length with handles at each end. One handle each hand, drape over your shoulder, saw back and forth. The only tool that properly cleans the back.',
    image: '/products/03-back-scrub-cloth.png',
    replacementCycleDays: 60,
    comingSoon: false,
  },
  {
    num: '04',
    name: 'Scalp Massager',
    fullName: 'Silicone Scalp Massager',
    origin: '🇰🇷 South Korea',
    tag: 'Scalp · Daily',
    tagline: 'Your scalp. Finally reached.',
    benefits: [
      'Made in Korea, best-grade silicone',
      'Built to last',
      'Firm enough to break up buildup, gentle enough for daily use',
      'Stimulates blood flow to follicles',
    ],
    desc: 'Best-grade silicone, made in Korea. Firm bristles that stimulate blood flow, break up scalp buildup and distribute shampoo properly. Built to last years.',
    image: '/products/04-scalp-massager.png',
    replacementCycleDays: 60,
    comingSoon: false,
  },
  {
    num: '05',
    name: 'Atlas Clay Mask',
    fullName: 'Atlas Clay Mask 300g',
    origin: '🇲🇦 Morocco',
    tag: 'Body · Weekly',
    tagline: 'Deep. Every week.',
    benefits: [
      'Draws out impurities',
      'Unclogs pores',
      'Improves skin clarity',
      'Gentle exfoliation',
      '100% organic clay',
    ],
    desc: 'Atlas mountain rhassoul clay. Draws out body impurities that daily washing never reaches. Applied head to toe. Leave 8–10 minutes.',
    image: '/products/05-atlas-clay.png',
    replacementCycleDays: 60,
    comingSoon: false,
  },
  {
    num: '06',
    name: 'Argan Body Oil',
    fullName: 'Organic Argan Body Oil 50ml',
    origin: '🇲🇦 Morocco',
    tag: 'Body · Weekly',
    tagline: 'Pure. Every week.',
    benefits: [
      'Deeply nourishing',
      'Fast absorbing',
      'Softens skin',
      'Antioxidant rich',
      '100% organic',
    ],
    desc: 'Cold-pressed. 100% certified organic. One ingredient. Applied on damp skin after weekly exfoliation. Absorption peaks on freshly cleared skin.',
    image: '/products/06-argan-oil.png',
    replacementCycleDays: 60,
    comingSoon: false,
  },
  {
    num: '07',
    name: 'Body Lotion',
    fullName: 'Daily Restore Body Lotion 200ml',
    origin: '🇬🇧 United Kingdom',
    tag: 'Body · Daily',
    tagline: 'Restore. Every day.',
    benefits: [
      'Restores moisture daily',
      'Absorbs in seconds',
      'Locks in moisture before it escapes',
      'Shields daily damage',
      'Soothes and softens',
    ],
    desc: 'Apply within 3 minutes of towelling. Skin absorbs 70% more moisture while still warm. No residue. Hydration that lasts past midday.',
    image: '/products/07-body-lotion.png',
    replacementCycleDays: 60,
    comingSoon: false,
  },
  {
    num: '08',
    name: 'Cleansing Cloth',
    fullName: 'Cleansing Cloth',
    origin: '🌿 Bamboo & Cotton',
    tag: 'Sensitive Areas · Daily',
    tagline: 'Sensitive areas. Deep hygiene.',
    benefits: [
      'For sensitive areas other tools never reach',
      'Deep cleaning hygiene, daily',
      'Made with bamboo and cotton',
      'Natural and biodegradable',
    ],
    desc: 'For sensitive areas other tools never reach. Deep cleaning hygiene every day. Made with bamboo and cotton. Natural, biodegradable, ultra-soft.',
    image: '/products/08-cleansing-cloth.png',
    replacementCycleDays: 60,
    comingSoon: false,
  },
  {
    num: '11',
    name: 'Clay Mixing Bowl',
    fullName: 'SOLUM Clay Mixing Bowl',
    origin: '',
    tag: 'Weekly · Tool',
    tagline: 'Mix. Apply. Done right.',
    benefits: [
      'Mix clay with argan oil evenly before applying',
      'Prevents mess on bathroom surfaces',
      'Sized for the right amount of clay per session',
    ],
    desc: 'Used to mix the clay with argan oil drops before applying. Keeps the process clean and controlled.',
    image: '/products/09-mixing-bowl.png',
    replacementCycleDays: 60,
    comingSoon: false,
  },
  {
    num: '09',
    name: 'Turkish Kese Mitt',
    fullName: 'Artisan Turkish Kese Mitt',
    origin: '🇹🇷 Turkey (Istanbul)',
    tag: 'Body · Weekly',
    tagline: 'The artisan upgrade.',
    benefits: [],
    desc: 'Hand-woven raw silk by artisans in Istanbul. Rougher than the Italy Towel Mitt. The deepest weekly exfoliation available. SOVEREIGN tier only.',
    image: null,
    replacementCycleDays: 60,
    comingSoon: true,
  },
  {
    num: '10',
    name: 'Beidi Black Soap',
    fullName: 'Beidi Black Soap',
    origin: '🇹🇷 Turkey',
    tag: 'Body · Weekly',
    tagline: 'Traditional hammam.',
    benefits: [],
    desc: 'Traditional hammam black soap from Turkey. Used for centuries. SOVEREIGN tier only.',
    image: null,
    replacementCycleDays: 60,
    comingSoon: true,
  },
];
