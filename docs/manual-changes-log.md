...
|------|--------|-------|----------------|
| 2026-08-12 | AWIN Task 7 worker recovery and schedule: a development bearer exposed in internal diagnostic output was treated as compromised and replaced; the replacement was synchronized to the Edge Function and the single Vault entry `awin_worker_bearer_dev` without exposing it. Created exactly one active `awin-conversion-worker-dev` pg_cron job at `* * * * *`, targeting only the exact DEV worker URL and reading the bearer from Vault. Guarded verification found a recent successful HTTP 200 empty run and an empty outbox; temporary files were removed. The isolated AWS DEV fixture and five DEV AWIN settings remain for acceptance. Read-only storefront checks found the Amplify DEV branch stale: no public MasterTag and `/feeds/awin.csv` returned 404, while `/buy`, `/checkout`, `/account`, and `/creators` correctly loaded no tag. Manual prerequisite: deploy/push `codex/awin-phase-a` to the Amplify DEV branch and repeat exact root/tag/feed checks before seeking production approval. | Supabase DEV `rodvvmfzkyjsqbufkjbc`, retained AWS `solum-awin-conversion-fixture-dev`, and read-only Amplify DEV `https://dev.d3pa095gzazg3c.amplifyapp.com` | No — production was not contacted or mutated; no DEV web deployment was performed. |
