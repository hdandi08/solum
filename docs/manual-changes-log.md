...
|------|--------|-------|----------------|
| 2026-08-12 | Restored Stripe dynamic payment methods on the final Payment Element page. The server-created PaymentIntent now allows Stripe to display all Dashboard-enabled methods eligible for the buyer, GBP transaction, device, and location (including Revolut Pay where eligible), while the first `/buy` Express Checkout surface remains separately controlled. Card remains available. | PaymentIntent Edge Function and regression coverage | DEV function/code first; production pending manual approval. |
| 2026-08-12 | Corrected the Instagram/social in-app-browser `/buy` presentation: the selected kit, total, included delivery, and card fallback are explicit; Stripe Express mounts immediately so any method supported by the webview can appear; the external-browser option remains available. The mobile sticky price bar is concealed while this decision card is open and uses hysteresis after continuing, removing overlap and toolbar-induced flicker. Existing Express/IAB PostHog event names are unchanged. | Web checkout UI and regression coverage | DEV web deployment first; production pending manual approval. |
